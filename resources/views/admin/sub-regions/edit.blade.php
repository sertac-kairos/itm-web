@extends('layouts.vertical', ['title' => 'Alt Bölge Düzenle - İzmir Time Machine', 'topbarTitle' => 'Alt Bölge Düzenle'])
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title">Alt Bölge Düzenle</h4>
        </div>
    </div>
</div>

<!-- Form -->
<form action="{{ route('admin.sub-regions.update', $subRegion) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    @if(isset($returnUrl))
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header">
                    <h4 class="header-title">Alt Bölge Bilgileri</h4>
                    <p class="text-muted mb-0">Alt bölgenin temel özelliklerini belirleyin</p>
                </div>
                <div class="card-body">
                    <!-- Dil Sekmeleri -->
                            ...
                            <li class="nav-item" role="presentation">
                                <a href="#{{ $locale }}-tab" data-bs-toggle="tab" aria-expanded="{{ $index === 0 ? 'true' : 'false' }}" 
                                   class="nav-link {{ $index === 0 ? 'active' : '' }}" role="tab">
                                    ...
                                    @if($subRegion->hasTranslation($locale))
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            <label for="{{ $locale }}_name" class="form-label">
                                                Alt Bölge Adı @if($locale === 'tr') <span class="text-danger">*</span> @endif
                                            ...
                                            <input type="text" class="form-control @error($locale.'.name') is-invalid @enderror" 
                                                   ...
                                                   value="{{ old($locale.'.name', $subRegion->translate($locale, false)?->name) }}">
                                            @error($locale.'.name')
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control @error($locale.'.subtitle') is-invalid @enderror" 
                                                   ...
                                                   value="{{ old($locale.'.subtitle', $subRegion->translate($locale, false)?->subtitle) }}">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control @error($locale.'.description') is-invalid @enderror" 
                                                      ...
                                                      rows="4" placeholder="Alt bölge hakkında açıklama yazın...">{{ old($locale.'.description', $subRegion->translate($locale, false)?->description) }}</textarea>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="text-muted mb-0">Alt bölgenin harita koordinatlarını belirleyin</p>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" step="any" class="form-control @error('latitude') is-invalid @enderror" 
                                           id="latitude" name="latitude" value="{{ old('latitude', $subRegion->latitude) }}" 
                                           placeholder="Örn: 38.4237" min="-90" max="90">
                                    <button type="button" class="btn btn-outline-primary" onclick="openMapModal()" title="Haritadan Seç">
                                        <i class="mdi mdi-map"></i>
                                    </button>
                                </div>
                                <small class="text-muted">-90 ile 90 arasında olmalı</small>
                                @error('latitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" step="any" class="form-control @error('longitude') is-invalid @enderror" 
                                           id="longitude" name="longitude" value="{{ old('longitude', $subRegion->longitude) }}" 
                                           placeholder="Örn: 27.1428" min="-180" max="180">
                                    <button type="button" class="btn btn-outline-primary" onclick="openMapModal()" title="Haritadan Seç">
                                        <i class="mdi mdi-map"></i>
                                    </button>
                                </div>
                                <small class="text-muted">-180 ile 180 arasında olmalı</small>
                                @error('longitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="header-title">Ana Bölge</h4>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="region_id" class="form-label">
                            Bağlı Olduğu Bölge <span class="text-danger">*</span>
                        </label>
                        <select class="form-select @error('region_id') is-invalid @enderror" 
                            ...
                            @foreach($regions as $region)
                                <option value="{{ $region->id }}" 
                                        {{ old('region_id', $subRegion->region_id) == $region->id ? 'selected' : '' }}
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if($subRegion->image)
                        <div class="mb-3">
                            ...
                            <div class="position-relative">
                                <img src="{{ asset('storage/' . $subRegion->image) }}" 
                                     alt="SubRegion Image" class="img-fluid rounded" style="max-height: 200px;">
                    ...
                    <!-- Resim -->
                    <div class="mb-3">
                        <label for="image" class="form-label">{{ $subRegion->image ? 'Yeni Resim' : 'Alt Bölge Resmi' }}</label>
                    ...
                    <!-- Renk -->
                    <div class="mb-3">
                        <label for="color" class="form-label">Alt Bölge Rengi</label>
                        <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror" 
                               id="color" name="color" value="{{ old('color', $subRegion->color ?? '#1a4a9f') }}">
                        <small class="text-muted">Alt bölge için tema rengi seçin</small>
                        @error('color')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('sort_order') is-invalid @enderror" 
                               id="sort_order" name="sort_order" value="{{ old('sort_order', $subRegion->sort_order) }}" min="0" required>
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input type="checkbox" class="form-check-input" id="is_active" name="is_active" 
                                   value="1" {{ old('is_active', $subRegion->is_active) ? 'checked' : '' }}>
                ...
                <div class="card-body">
                    <div class="preview-card p-3 border rounded" style="background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%);">
                        <div class="d-flex align-items-center mb-2">
                            <span class="region-color-preview me-2" style="background-color: {{ $subRegion->region->color_code }}; width: 16px; height: 16px; border-radius: 50%; display: inline-block;"></span>
                            <h6 class="mb-0 subregion-name-preview">{{ $subRegion->name ?: 'Alt Bölge Adı' }}</h6>
                        </div>
                        <p class="text-muted mb-2 subregion-desc-preview small">{{ $subRegion->description ?: 'Alt bölge açıklaması burada görünecek...' }}</p>
                        <div class="text-muted small mb-2">
                            <i class="mdi mdi-map-marker me-1"></i>
                            <span class="coordinates-preview">
                                @if($subRegion->latitude && $subRegion->longitude)
                                    {{ number_format($subRegion->latitude, 6) }}°, {{ number_format($subRegion->longitude, 6) }}°
                                @else
                                    Koordinat bilgisi
                                @endif
                            </span>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h6 class="mb-0">Alt Bölge Güncelleniyor</h6>
...
<script>
    ...
    function resetMap() {
        ...
        // Clear search input
        const searchInput = document.querySelector('#siteMap input[type="text"]');
        if (searchInput) {
            searchInput.value = '';
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        regionSelect.addEventListener('change', updateRegionColor);
        ...
        function updatePreview() {
            ...
            document.querySelector('.subregion-name-preview').textContent = nameInput.value || 'Alt Bölge Adı';
            document.querySelector('.subregion-desc-preview').textContent = descInput.value || 'Alt bölge açıklaması burada görünecek...';
            document.querySelector('.subregion-order-preview').textContent = 'Sıra: ' + sortOrder;
            
            // Coordinates preview
            let coordsText = 'Koordinat bilgisi';
            if (latitude || longitude) {
                coordsText = `${latitude || '0'}°, ${longitude || '0'}°`;
            }
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            if (hasError) {
                e.preventDefault();
                alert('En az bir dilde alt bölge adı girilmelidir!');
            }
        });
    ...
    function translateFromTurkish() {
            ...
            if (!turkishName || !turkishName.value.trim()) {
                alert('Lütfen önce Türkçe alt bölge adı girin.');
                return;
            }
            ...
            fetch('/admin/sub-regions/translate', {
                method: 'POST',
                headers: {
